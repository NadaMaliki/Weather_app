...
/**
 * Classe pour gérer les préférences des utilisateurs, incluant les unités et les villes favorites.
 */
public class UserPreferences {
    ...
    private DAO.UserPreferencesManager dbManager;
   ...
   //getters and setters
       ...
       public List<String> getFavoriteCities () {return favoriteCities;}
       ...
       try (Connection conn = dbManager.getCon();
           ...
           if (rs.next()) {
               ...
               unit = rs.getString("Unite") != null ? rs.getString("Unite") : "°C";  // Valeur par défaut si l'unité est nulle
           }
    ...
    public boolean editUnit(int userId, String unit) {
        // Optionnel : Valider les valeurs autorisées
        if (!unit.equals("°C") && !unit.equals("°K") && !unit.equals("°F")) {
            throw new IllegalArgumentException("L'unité doit être Celsius, Kelvin ou Fahrenheit.");
        }
